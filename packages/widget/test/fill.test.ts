...
test('text input: native PROTOTYPE setter is used (not el.value =) and input+change fire in order', () => {
  ...
  const report = applyFillPlan(plan({ fieldId: 'ff-0', action: 'set', value: 'Ada' }), resolve);
...
test('the dispatched input event is an InputEvent with insertText + composed', () => {
  ...
  applyFillPlan(plan({ fieldId: 'ff-0', action: 'set', value: 'x@y.z' }), resolve);
...
test('readback-or-revert: a value the element rejects is restored and reported left', () => {
...
test('checkbox: click() only when the desired state differs from current', () => {
...
test('radio group: clicks the member whose value matches the plan', () => {
  const { resolve } = mount(
  ...
  const report = applyFillPlan(plan({ fieldId: 'ff-0', action: 'set', value: 'pro' }), resolve);
...
test('select single: sets option.selected and fires change', () => {
  const { resolve, byId } = mount(
    `<select id="c" name="c">
  ...
  const report = applyFillPlan(plan({ fieldId: 'ff-0', action: 'set', value: 'de' }), resolve);
...
test('select multiple: selects every planned value', () => {
  const { resolve, byId } = mount(
...
test('skip action and omitted fields are left completely untouched', () => {
  const { resolve, byId } = mount(
  ...
  const report = applyFillPlan(plan({ fieldId: 'ff-0', action: 'skip', value: null }), resolve);
...
test('an unresolved field id is recorded left, not thrown', () => {
...
test('the form is NEVER submitted during a fill', () => {
  ...
  applyFillPlan(plan({ fieldId: 'ff-0', action: 'set', value: 'value' }), resolve);
  void byId;
...
test('a non-fillable element (readonly) is left, not written', () => {
  const { resolve, byId } = mount(`<input id="ro" name="ro" value="locked" readonly />`, {
    'ff-0': '#ro',
  });
  const input = byId.get('ff-0') as HTMLInputElement;

  const report = applyFillPlan(plan({ fieldId: 'ff-0', action: 'set', value: 'new' }), resolve);
