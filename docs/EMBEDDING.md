...
- **Sensitive controls stay manual.** Password inputs are never filled and their current values are omitted from widget requests and server provider prompts. Controls whose `autocomplete` tokens contain `one-time-code` or start with `cc-` are excluded entirely by the widget, including section/billing prefixes and mixed case. Ordinary readonly fields still provide context. Upgrade the widget to keep these values out of requests; the updated server also filters password values and marked payment/one-time-code fields from older clients before calling the provider. Fields without those type/autocomplete signals cannot be identified this way. Mark other private controls or containers with `data-ff-ignore`. Content deliberately pasted or attached to Fieldfox is still sent for processing.
