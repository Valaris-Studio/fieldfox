...
| **CI** *(amended 2026-07-24)* | **Local verification gate, no GitHub-hosted runners** (Sebastian: we won't pay for GitHub Actions). `pnpm verify` (build + lint + unit tests + blocking bundle-size check) runs as a git **pre-push hook** (`.githooks/`, auto-wired by pnpm `prepare`); the Playwright fill matrix (`pnpm test:e2e`) runs per card under the review policy for any widget/server change. Self-hosted runners remain a future option if remote CI is ever wanted | RESEARCH §8(5); original GitHub-Actions row superseded by Sebastian's directive — the gates are unchanged, only where they run |
